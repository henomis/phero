...
// Tool is a tool that allows running Go commands.
type Tool struct {
...
// path specifies the working directory for the Go command.
func New() (*Tool, error) {
	name := "go"
	description := "use this tool to run go command."
...
// WithValidation allows setting a custom validation function for the GoTool.
func (g *Tool) WithValidation(validateFn func(context.Context, *Input) error) *Tool {
	g.validateFn = validateFn
	return g
}

// WithWorkingDirectory sets the working directory for the GoTool.
func (g *Tool) WithWorkingDirectory(workingDirectory string) *Tool {
	g.workingDirectory = workingDirectory
	return g
}

// Tool returns the llm.FunctionTool representation of the GoTool.
...
func (g *Tool) run(ctx context.Context, input *Input) (*Output, error) {
	...
	cmd := exec.CommandContext(ctx, "go", input.Args...)
