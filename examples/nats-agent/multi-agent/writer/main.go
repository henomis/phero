...
// Package main runs the Writer NATS agent.
//
// The writer takes research notes and a topic, and produces a polished
// 300-500 word article. It is part of the multi-agent NATS newsroom example.
//
// Start with:
//
//	OPENAI_API_KEY=... go run ./examples/nats-agent/multi-agent/writer
//
// The agent registers under owner="newsroom", name="writer" on the NATS bus.
package main
...
Use active voice, avoid jargon, and make complex ideas accessible to a general audience.`,
	...
	srv, err := natsagent.New(nc, writer, owner, name,
